...
export function printTodos() {
  getDocs(sortQuery).then((snapshot) => {
    ...
    todos.map((todo) => {
      ...
      moment(todo.Date).diff(moment(), "days") <= 1
        ? (date.style.color = "red")
        : moment(todo.Date).diff(moment(), "days") >= 7
      ...
      svgCheckedPath.setAttribute("d", "M4.5 12.75l6 6 9-13.5");
      ...
      let titleListener;
      let confirmListener;
      let descriptionListener;
      let dateListener;
      if (!todo.Done) {
        titleListener = title.addEventListener("click", () => {
          toggle();
          toggler
            ? (confirmBtn.style.display = "none")
        ...
        descriptionListener = description.addEventListener("click", () => {
          toggle();
          toggler
            ? (confirmBtn.style.display = "none")
            : (confirmBtn.style.display = "block");
        ...
        dateListener = date.addEventListener("click", () => {
          toggle();
          toggler
            ? (confirmBtn.style.display = "none") &&
              (editDate.style.display = "none")
        ...
        confirmListener = confirmBtn.addEventListener("click", (e) =>
          editTodo(e, title.innerText, description.innerText, editDate.value)
        );
      }
      if (todo.Done) {
        title.removeEventListener("click", titleListener);
        title.removeEventListener("click", descriptionListener);
        title.removeEventListener("click", dateListener);
        title.removeEventListener("click", confirmListener);
      }
